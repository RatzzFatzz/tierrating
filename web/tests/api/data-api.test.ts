...
describe("fetchData", () => {
  it("throws when token is null", async () => {
    await expect(fetchData(null, "user", "anilist", "anime")).rejects.toThrow(
...
describe("updateData", () => {
  it("throws when token is null", async () => {
    await expect(updateData("entry-1", 9, "anilist", "anime", null, "testuser")).rejects.toThrow(
      "No authentication token"
    );
    expect(mockFetch).not.toHaveBeenCalled();
  });
...
describe("pullData", () => {
  it("throws when token is null", async () => {
    await expect(pullData(null, "testuser", "anilist", "anime")).rejects.toThrow(
      "No authentication token"
    );
    expect(mockFetch).not.toHaveBeenCalled();
  });
  ...
  it("includes username, service and type in URL path", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining("/data/pull/alice/trakt/tvshows"),
      expect.any(Object)
    );
  });
});
