...
describe("fetchTiers", () => {
  it("throws when token is null", async () => {
    await expect(fetchTiers(null, "testuser", "anilist", "ANIME")).rejects.toThrow(
  ...
  it("lowercases the type in the URL", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining("/tiers/user/trakt/movies"),
      expect.any(Object)
    );
  });
...
describe("updateTiers", () => {
  it("throws when token is null", async () => {
    await expect(
      updateTiers(null, "testuser", "anilist", "anime", [createMockTier()])
  ...
  it("returns null data when json parsing fails", async () => {
    ...
    const result = await updateTiers("my-token", "testuser", "anilist", "anime", [
      createMockTier(),
    ]);
